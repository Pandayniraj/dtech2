        ...
        <div class='col-md-12 box'>
            <div class="box-body">
                ...
                <div class="col-md-12">
                        ...
                        <form method="POST" action="/admin/supplierreturn">
                            {{ csrf_field() }}
                             ...
                             <div class="row">
                                ...
                                <div class="col-md-12">
                                    <div class="col-md-3">
                                        ...
                                        {{-- class="js-example-basic-multiple" name="states[]" multiple="multiple" --}}
                                          <select class="form-control pull-right js-example-basic-multiple"  name="purchase_bill_id[]" value="" id="purchase_bill_no" required="" multiple="multiple">
                                     ...
                                     <div class="col-md-3">
                                        <label>Purchase bill No</label>
                                          <input type="text" class="form-control pull-right " name="purchase_bill_no[]" id="pur_billno" value="" required="" readonly>
                                     </div>
                                      <div class="col-md-3">
...
//     });
   ...
   $('#purchase_bill_no').on('change',function(){
         $.ajax(
            {
             ...
             dataType: "json", 
                success: function( data ) { 
                    ...
                    $('#pur_billno').val(data.bill_no).prop("readonly", true);
                    $('#purchase_order_date').val(data.purchaseorderdate).prop("readonly", true); 
                    $('#user_id').val(data.purchaseowner).prop("readonly", true); 
                    $('#pan_no').val(data.panno).prop("readonly", true); 
                    $('#customer_name').val(customer_name).prop("readonly", true);
                    $('#client_id').val(data.customer_id);
                    $(".multipleDiv").after(purchasedetailinfo);

                    $('#subtotal').val(data.subtotal);
                    $('#sub-total').html(data.subtotal);

                    $('#taxableamount').val(data.taxableamt);
                    $('#taxable-amount').html(data.taxableamt);

                    $('#taxabletax').val(data.taxamt);
                    $('#taxable-tax').html(data.taxamt);

                    $('#total').html(data.total);
                    $('#total_').val(data.total);
                } 
            }); 
    ...
    function populatePurchaseInfo(invoiceinfo){
        ...
        //  var bill_no=document.getElementById("pur_billno").value;
        // invoiceinfo.forEach(function(item) {
        //     if(bill_no){
        //     $('#pur_billno').val( bill_no + ',' +item.bill_no).prop("readonly", true); 
        // }
        // else{
        //     $('#pur_billno').val( item.bill_no).prop("readonly", true);  
        // }
        // });
