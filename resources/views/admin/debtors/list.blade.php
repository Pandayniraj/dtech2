                                  ...
                                  <table class="table table-hover table-striped table-responsive" id="clients-table">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td style="text-align: center;font-size: 16.5px">
                                                {{$entries['amount']<0?'Cr.'.number_format(abs($entries['amount']),2):number_format($entries['amount'],2)}}
                                        </td>
                                            @if($current_fiscal->fiscal_year==$fiscal_year)
                                        ...
                                        @php
                                        $totalSum +=  $entries['amount'];
                                        $totaldr +=  $entries['dr_amount'];
                                        $totalcr +=  $entries['cr_amount'];
                                        $totalopeningdr +=  $entries['opening_blc_dc']=='D'?$entries['opening_blc']:0;
                                        $totalopeningcr +=  $entries['opening_blc_dc']=='C'?$entries['opening_blc']:0;
                                        @endphp
                                        @endforeach
                                    </tbody>
                                    <tfoot>
                                        <tr>
                                            <td class="text-right" colspan="3" style="font-size: 20.5px">Total {{ env('APP_CURRENCY') }}</td>
                                            <td class="text-right" style="font-size: 20.5px;">{{number_format(isset($totalopeningdr)?$totalopeningdr:0,2)}}</td>
                                            <td class="text-right" style="font-size: 20.5px;">{{number_format(isset($totalopeningcr)?$totalopeningcr:0,2)}}</td>
                                            <td class="text-right" style="font-size: 20.5px;">{{number_format(isset($totaldr)?$totaldr:0,2)}}</td>
                                            <td class="text-right" style="font-size: 20.5px;">{{number_format(isset($totalcr)?$totalcr:0,2)}}</td>
                                            <td class="text-right" style="font-size: 20.5px;"> {{ number_format(isset($totalSum)?$totalSum:0,2) }} </td>
                                        </tr>
                                    </tfoot>
                                </table>
                            </div>
