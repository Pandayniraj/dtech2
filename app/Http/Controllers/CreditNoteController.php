...
class CreditNoteController extends Controller
{
    ...
    public function getInvoiceId()
    {
        ...
        $salesinvoice = \App\Models\Invoice::select('id')->where('id', 'LIKE', '%'.$term.'%')->take(5)->get();
        $return_array = [];
...
public function getbillinfo(Request $request){
    ...
    {
        $invoiceinfo = \App\Models\Invoice::find($request->salesinvoice_id);
        ...
        // dd($customer_name);
        $invoicedetailinfo = \App\Models\InvoiceDetail::where('invoice_id', $invoiceinfo->id)->get();
        ...
        foreach ($invoicedetailinfo as $idi) {
            if ($idi->is_inventory == 1) {
                ...
                $data .= '<tr>  
                                ...
                                <td>
                                    <input type="number" class="form-control price" name="credit_price[]" placeholder="Credit Price" step="0.01" min="1" value="'.$idi->price.'" required="required" >
                                </td>
