...
//    }
    ...
    public function DownloadExcel($label, $id)
    {
        ...
        return \Excel::download(new \App\Exports\ExcelExport($data), 'entry_'.$label.'_'.$entries->id.'.csv');
    }
    public function PurchaseDownloadExcel($label, $id)
    {

        $entries = Entry::where('id', $id)->first();
        // dd($entries);
        $data = Entryitem::orderBy('id', 'asc')->where('entry_id', $entries->id)->get()->toArray();
        return \Excel::download(new \App\Exports\PurchaseExcelExport($id), 'entry_'.$label.'_'.$entries->id.'.xlsx');
    }
    public function SamplePurchaseDownloadExcel()
    {
        return \Excel::download(new \App\Exports\SamplePurchaseExcelExport(), 'SampleExcel'.'.xlsx');
    }
    public function purchaseimportexcel(Request $request){
        if ($request->hasFile('import_file')) {
            $file = $request->file('import_file');
            $fiscal_year_id = \FinanceHelper::cur_fisc_yr()->id;
            $import= \Excel::import(new \App\Imports\PurchaseExcelImport($fiscal_year_id), $file);
