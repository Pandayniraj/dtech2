...
public static function getLedgerDrCr($ledger, $start_date = null, $end_date = null)
{
...
public static function getDrCrByGroups($id,$start_date = null, $end_date = null)
{
    ...
    // dd($start_date,$end_date,$dr_cr_amount);
    // if ($id == 286){
...
public static function getTotalByGroups($id, $start_date = null, $end_date = null)
    {
        $start_date = Request::get('start_date') ?? $start_date;
        $end_date = Request::get('end_date') ?? $end_date;
        $ei_table = new Entryitem();
        $ledger_table = new COALedgers();
        $group_table = new COAgroups();

            $current_fiscal = Fiscalyear::where('current_year', 1)->first();


        // build our category list only once//anamoldoing
        $cats = [];
        ...
        ->whereIn( 'coa_ledgers.group_id', $parent_ids)
        ->when($start_date && $end_date, function ($q) use ($start_date, $end_date) {
            $q->whereDate( 'entries.date', '>=', $start_date)
            ->whereDate( 'entries.date', '<=', $end_date)
            ->where( 'entries.is_approved', '=', 1);
        })
        ->first();
        $opening_balance = $ledger_table->select(
            DB::raw('SUM(if (op_balance_dc="D", op_balance, 0)) as dr_opening_balance, SUM(if (op_balance_dc="C", op_balance, 0)) as cr_opening_balance')
        )->whereIn('group_id', $parent_ids)->first();
        $op_blc['dc']='C';
        if($opening_balance->dr_opening_balance>$opening_balance->cr_opening_balance){
            $op_blc['dc']='D';
        }
        $op_blc['amount']=abs($opening_balance->dr_opening_balance-$opening_balance->cr_opening_balance);
...
//            ->where('fin_order_detail.product_id','!=',0)
    ...
    public static function getLedgersOpeningBalance($ledger, $start_date)
{
    ...
    $ledger_bal=$ledger_table->find($ledger->id);
    $data['dc']=$ledger_bal->op_balance_dc;
    $data['amount']=$ledger_bal->op_balance;
    return $data;
}
